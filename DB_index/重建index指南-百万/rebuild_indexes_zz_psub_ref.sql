-- ============================================================================
-- 重建索引完整流程（ZZ_PSUB_REF_diff_temp表）
...
PROMPT ======================================================================
PROMPT 开始重建ZZ_PSUB_REF_diff_temp表的所有索引
PROMPT 执行时间：
...
CREATE TABLE idx_backup_zz_psub_ref_$(TO_CHAR(SYSDATE,'YYYYMMDD_HH24MISS')) AS
SELECT
    index_name,
    index_type,
    uniqueness,
...
-- 【核心索引1】函数索引 - 支持按日期过滤
PROMPT 创建索引1/5: idx_ZZ_PSUB_REF_diff_temp_trunc_date...
CREATE INDEX idx_ZZ_PSUB_REF_diff_temp_trunc_date
ON ZZ_PSUB_REF_diff_temp(TRUNC(create_date));
PROMPT ✓ 完成

-- 【核心索引2】复合索引 - 支持导出查询
PROMPT 创建索引2/5: idx_ZZ_PSUB_REF_diff_temp_export_main...
CREATE INDEX idx_ZZ_PSUB_REF_diff_temp_export_main
ON ZZ_PSUB_REF_diff_temp(
    TRUNC(create_date),
    compare_status
);
PROMPT ✓ 完成
...
-- 【辅助索引3】比较状态索引
PROMPT 创建索引3/5: idx_ZZ_PSUB_REF_diff_temp_status...
CREATE INDEX idx_ZZ_PSUB_REF_diff_temp_status
ON ZZ_PSUB_REF_diff_temp(compare_status);
PROMPT ✓ 完成

-- 【辅助索引4】旧系统数据索引
PROMPT 创建索引4/5: idx_ZZ_PSUB_REF_diff_temp_old_data...
CREATE INDEX idx_ZZ_PSUB_REF_diff_temp_old_data
ON ZZ_PSUB_REF_diff_temp(
    compare_status,
    Subscriber,
    Cellular
...
-- 【辅助索引5】新系统数据索引
PROMPT 创建索引5/5: idx_ZZ_PSUB_REF_diff_temp_new_data...
CREATE INDEX idx_ZZ_PSUB_REF_diff_temp_new_data
ON ZZ_PSUB_REF_diff_temp(
    compare_status,
    Subscriber_NB,
    Cellular_NB
