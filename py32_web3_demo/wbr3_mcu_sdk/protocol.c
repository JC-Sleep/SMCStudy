...
#include "wifi.h"
#include "uart.h"   /* UART1_SendByte() — PY32F030 移植接口 */

/* 最近一次 DHT11 传感器读数，定义在 main.c，此处声明引用 */
extern volatile uint8_t g_last_temp;
extern volatile uint8_t g_last_humi;
...
void uart_transmit_output(u8 value) {
    UART1_SendByte(value);   /* PY32F030 USART1 单字节阻塞发送 */
}
...
void all_data_update(void){
    /* 上报最近一次 DHT11 读数到涂鸦云（App 查询或云端请求时触发） */
    mcu_dp_value_update(DPID_TEMP_CURRENT,   (u32)g_last_temp);
    mcu_dp_value_update(DPID_HUMIDITY_VALUE, (u32)g_last_humi);
}
